...
static Eternal<Function> fast_buffer_constructor;
...
static bool calc_ebml_size(Isolate *isolate, Handle<Value> val, size_t &size)
{
    ...
    Handle<Array> tags = val.As<Array>();
    uint32_t length = tags->Length();
    for (uint32_t i = 0; i < length; i++) {
        Handle<Value> tagVal = tags->Get(i);
        if (!tagVal->IsArray()) {
            throw_error(isolate, "Invalid tag");
            return false;
        }

        Handle<Array> tag = tagVal.As<Array>();
        ...
        Handle<Value> idVal = tag->Get(0);
        Handle<Value> typeVal = tag->Get(1);
        Handle<Value> contentVal = tag->Get(2);
        ...
        size_t idSize = Buffer::Length(idVal);
...
static bool write_ebml(Isolate *isolate, Handle<Value> val, uint8_t *dst)
{
    ...
    uint32_t length = tags->Length();
    for (uint32_t i = 0; i < length; i++) {
        Handle<Array> tag = tags->Get(i).As<Array>();
...
static void write_ebml(const FunctionCallbackInfo<Value>& args)
{
    ...
    Handle<Object> bufobj = fast_buffer_constructor.Get(isolate)->NewInstance(1, &arg);
    ...
    v8::Handle<v8::Context> context, void* priv)
{
    ...
    Local<Value> val = context->Global()->Get(String::NewFromUtf8(isolate, "Buffer"));
    fast_buffer_constructor.Set(isolate, val.As<Function>());

    NODE_SET_METHOD(exports, "writeEBML", write_ebml);
}

NODE_MODULE_CONTEXT_AWARE(ebml, init)
